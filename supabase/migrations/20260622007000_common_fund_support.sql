...
alter table public.ledger_entries
  ...
  check (type in ('deposit', 'event-share', 'pending', 'balance-donation', 'common-fund-support'));
...
begin
  ...
  with ledger_totals as (
    select
      ...
      coalesce(sum(le.amount) filter (where le.type in ('event-share', 'common-fund-support')), 0)::numeric as spent,
