...
alter table public.ledger_entries
  ...
  check (type in ('deposit', 'event-share', 'pending', 'balance-donation', 'common-fund-support', 'balance-refund'));
...
begin
  ...
  with ledger_totals as (
    select
      ...
      coalesce(sum(le.amount) filter (where le.type in ('event-share', 'common-fund-support', 'balance-refund')), 0)::numeric as spent,
      coalesce(sum(le.amount) filter (where le.type = 'balance-donation'), 0)::numeric as donated,
      coalesce(sum(le.amount) filter (where le.type = 'common-fund-support'), 0)::numeric as supported
  ...
  my_totals as (
    select
      coalesce(sum(le.amount) filter (where le.type = 'deposit'), 0)::numeric as deposited,
      coalesce(sum(le.amount) filter (where le.type in ('event-share', 'balance-donation', 'balance-refund')), 0)::numeric as spent
    from public.ledger_entries le
    where le.fund_id = target_fund_id
  ...
  select
    ...
    greatest(0, ledger_totals.donated - ledger_totals.supported),
    current_member_id,
    my_totals.deposited,
    my_totals.spent,
